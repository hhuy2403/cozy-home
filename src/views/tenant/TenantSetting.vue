  ...
  methods: {
    ...
    async updateSettings() {
      ...
      try {
        ...
        const currentUserEmail = this.tenant.email;

        // Cập nhật thông tin trong homes
        homes.forEach(home => {
          home.rooms.forEach(room => {
            if (room.customer && room.customer.email === currentUserEmail) {
              room.customer.fullName = this.tenant.fullName;
              room.customer.phoneNumber1 = this.tenant.phoneNumber;
              room.customer.address = this.tenant.address;

              if (this.newPassword) {
                room.customer.password = this.newPassword;
              }
            }
          });
        });
        ...
        Swal.fire('Lỗi', 'Không thể cập nhật thông tin tài khoản', 'error');
      } finally {
        this.loading = false;
      }
    },
    ...
    handleAvatarChange(event) {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          this.tenant.avatar = e.target.result;
        };
