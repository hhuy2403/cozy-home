  ...
  methods: {
    async fetchData() {
      try {
        ...
        // Fetch data không có query params
        const [roomsRes, billsRes, bookingsRes, contractsRes] = await Promise.all([
          fetch(`${API_URL}/rooms`),
          fetch(`${API_URL}/bills`),
          fetch(`${API_URL}/landlord-bookings`),
          fetch(`${API_URL}/contracts`)
        ]);

        // Kiểm tra response
        if (!roomsRes.ok || !billsRes.ok || !bookingsRes.ok || !contractsRes.ok) {
          throw new Error('Lỗi khi tải dữ liệu');
        }

        // Parse và lọc dữ liệu theo landlordId
        const [rooms, bills, bookings, contracts] = await Promise.all([
          roomsRes.json(),
          billsRes.json(),
          bookingsRes.json(),
          contractsRes.json()
        ...
        this.rooms = rooms.filter(room => room.landlordId === currentUser.id);
        this.bills = bills.filter(bill =>
          bill.landlordId === currentUser.id &&
          bill.month === currentMonthYear
        );
        this.bookings = bookings.filter(booking => booking.landlordId === currentUser.id);
        this.contracts = contracts.filter(contract => contract.landlordId === currentUser.id);
