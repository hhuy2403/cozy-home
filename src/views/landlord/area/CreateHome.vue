...
export default {
  ...
  data() {
    return {
      home: {
        ...
        ward: "",
        address: "",
        landlordId: this.getLandlordId(), // Store landlordId from current user
      },
      errors: {},
    };
  },
  methods: {
    getLandlordId() {
      ...
      return currentUser && currentUser.role === "landlord" ? currentUser.id : null;
    },
    ...
    async validateAndSave() {
      ...
      if (Object.keys(this.errors).length === 0) {
        // Check for duplicate house name before saving
    ...
    async checkDuplicateName() {
      try {
        // Fetch homes from MockAPI to check if the name is already taken
        ...
        if (duplicateHome) {
          ...
          // No duplicate found, proceed to save the home
          this.saveHome();
        }
      } catch (error) {
        console.error("Lỗi khi kiểm tra tên nhà:", error);
        this.errors.general = "Không thể kiểm tra tên nhà. Vui lòng thử lại.";
      }
    },

    async saveHome() {
      try {
        const response = await crudApi.create('api::home.home', this.home);

        if (!response.error) {
          Swal.fire({
            icon: 'success',
            title: 'Lưu thành công!',
            text: 'Nhà đã được lưu thành công!',
          ...
          this.errors.general = 'Lỗi khi lưu nhà. Vui lòng thử lại.';
        }
      } catch (error) {
        console.error("Lỗi khi lưu nhà:", error);
