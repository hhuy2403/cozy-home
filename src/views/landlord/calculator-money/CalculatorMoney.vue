  ...
  computed: {
    ...
    filteredRooms() {
      return this.rooms.filter(room =>
        !this.calculationHouse || room.houseId === this.calculationHouse
      );
    }
  },
    ...
    async calculateBills() {
      try {
        ...
        const selectedRooms = this.rooms.filter(room => {
          if (this.calculationHouse && room.houseId !== this.calculationHouse) return false;
        ...
        for (const room of selectedRooms) {
          ...
          const electric = this.electricData.find(e =>
            e.roomId === room.id && e.monthYear === this.calculationMonth
          );

          const water = this.waterData.find(w =>
            w.roomId === room.id && w.monthYear === this.calculationMonth
          );

          const otherFees = this.otherFees.filter(f =>
        f.houseId === room.houseId && 
        ...
        f.monthYear === this.calculationMonth
          ...
          const existingBill = existingBills.find(
            bill => bill.roomId === room.id && bill.month === this.calculationMonth
          );
